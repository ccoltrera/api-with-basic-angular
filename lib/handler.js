...
function getEntries(req, res) {
  Entry.find({ $query: {}, $orderby: { datePosted : -1 } }, function(err, entries) {
    if (err) res.sendStatus(500);
    else {
      res.json(entries);
    }
...
function putEntryById(req, res) {
  ...
  Entry.find({_id: entry_id}, function(err, entries) {
    if (err) res.sendStatus(500);
      ...
      Entry.findOneAndUpdate({_id: entry_id}, {entryBody: req.body.entryBody, title: req.body.title, dateEdited: req.body.dateEdited}, {overwrite: false, new: true}, function(err, editedEntry) {
        if (err) {
          res.sendStatus(500);
        } else {
